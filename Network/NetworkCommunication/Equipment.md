...
##### 1.1.2.1 라우팅 테이블 정보 
메트릭(Metric) : 라우터가 목적지에 이르는 여러 경로 중의 최적의  - 경로를 결정하는데, 우선 순위의 기준의 매길수있게 숫자로 환산하는 변수. (값이 낮을수록 우선순위 크다.)
...
1. Maximum Hop count
라우터의 최대 홉 카운트는 15이다. 16부터는 라우팅이 되지 않는 것을 이용하는 것. => 대규모 망에서는 홉이 많기 때문에 사용안함.
2. Hold Down Timer
어떠한 경로가 Down 상태일 경우. 테이블 값을 바로 변경하지 않고 *일정 시간* 이 지난 후에 변경하는 것. 
=> 일정시간이란, 모든 라우터에 해당 경로가 down되었다는것을 인식한 시간 
3. Split Horizon
두 라우터 간에 루핑만을 막아주기 위한 기술.
자신 보다 메트릭값이 큰 네트워크의 정보를 보내준 라우터에 정보 송신시 해당 네트워크에 대한 정보를 보내지 않는다. 
4. Route Poisoning
라우터가 한 네트워크가 다운 되었을 경우, 메트릭 값을 변경하여 사용할 수없게 만드는 것이다. 
즉, 다운된 네트워크를 무한 대치상태로 바꾸어 라우팅 테이블에서 지운후, 잘못된 라우팅 정보를 받는 일을 사전에 방지
5. Poison Reverse
라우팅 정보를 보내온 쪽으로 되돌려 보낼때 이값을 최대치로 쓴다. 
### 라우터 경로 찾기 tracert(cisco)
``tracert 목적지IP``
 목적지 IP로 가는 경로들의 게이트웨이의 목록을 보여준다.
TTL 값을 1로 시작하여 1개씩 증가 시켜 홉들간에 연결상태를 추적하여 경로를 찾을수있다.라우터 IP 주소 할당
